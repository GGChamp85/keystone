...
| Hugging Face Hub (all model loads) | `HF_HUB_OFFLINE=1`, `TRANSFORMERS_OFFLINE=1` | `helm/keystone/templates/configmap.yaml`, `helm/keystone/templates/vllm.yaml`, `helm/keystone/templates/kuberay.yaml` (production default; `.env.example`'s dev default is `HF_HUB_OFFLINE=0` so a developer's first `docker compose up` can still pull a model without pre-seeding — flip to `1` for any air-gapped or otherwise network-restricted run) |
