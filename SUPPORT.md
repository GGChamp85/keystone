# Support

## Getting help

- **Documentation**: start at [`docs/`](docs/) — see the README's Documentation table for the right page (deployment, sandbox architecture, licenses, telemetry audit).
- **Bugs**: open an issue using the [bug report template](.github/ISSUE_TEMPLATE/bug_report.yml).
- **Feature requests**: open an issue using the [feature request template](.github/ISSUE_TEMPLATE/feature_request.yml).
- **Questions / usage help**: use [GitHub Discussions](../../discussions) if enabled, or the [question template](.github/ISSUE_TEMPLATE/question.yml).
- **Security vulnerabilities**: see [`SECURITY.md`](SECURITY.md) — do not use a public issue.

## Before asking

Run `keystone doctor` (or, until that ships, check `docs/deployment/`) and include its output — most deployment questions turn out to be a missing env var, an unreachable service, or an air-gap egress rule, and `doctor`/the relevant runbook usually names the exact problem.

## Commercial support

Not currently offered. This is an open-source project maintained on a best-effort basis.
